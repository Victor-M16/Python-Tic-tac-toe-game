...
class Board:
	...
	def check_for_winner(self, player):
		...
		c = player.counter
		for i in range(3):
			if self.cell_contains(c, i, 0) and self.cell_contains(c, i, 1) and self.cell_contains(c, i, 2):
				return c
			if self.cell_contains(c, 0, i) and self.cell_contains(c, 1, i) and self.cell_contains(c, 2, i):
				return c
		if self.cell_contains(c, 0, 0) and self.cell_contains(c, 1, 1) and self.cell_contains(c, 2, 2):
			return c
		if self.cell_contains(c, 0, 2) and self.cell_contains(c, 1, 1) and self.cell_contains(c, 2, 0):
			return c
		return None
	...
	def play(self):
		...
		while self.winner is None:
			...
			if self.next_player == self.human:
				...
				self.winner = self.board.check_for_winner(self.next_player)
				if self.winner is not None:
					print(str(self.next_player) + ' wins!')
					break
				else:
					self.next_player = self.computer
			# Computers move
